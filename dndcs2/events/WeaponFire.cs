...
public class WeaponFire : DndEvent<EventWeaponFire>
{
    ...
    public override HookResult DefaultPreHookCallback(EventWeaponFire @event, GameEventInfo info)
    {
        if(@event.Userid == null || @event.Userid.TeamNum == 0)
            return HookResult.Continue;

        var player = @event.Userid;
        var item = @event.Weapon.Replace("weapon_", "");
